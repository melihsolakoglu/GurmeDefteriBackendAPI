...
import com.example.gurmedefteri.ui.adapters.pagination.CategorizedScoredFoodPagination
...
class ScoredFoodViewModel @Inject constructor(
    ...
    init {
        ...
        checkUserIdExistsInScoredFoods()
    }
    ...
    fun controlMainFoodsLists(){
        CoroutineScope(Dispatchers.Main).launch {
            try {
                val response = krepo.getScoredCategorizeFoodsByPageWithUserId(userId.value.toString(),"Ana Yemek",1, 7)
                if(response.isSuccessful){
                    mainFood.value =true
    ...
    fun controlSoapFoodsLists(){
        CoroutineScope(Dispatchers.Main).launch {
            try {
                val response = krepo.getScoredCategorizeFoodsByPageWithUserId(userId.value.toString(),"Çorba",1, 7)
                if(response.isSuccessful){
                    soapFood.value = true
    ...
    fun controlDesertsFoodsLists(){
        CoroutineScope(Dispatchers.Main).launch {
            try {
                val response = krepo.getScoredCategorizeFoodsByPageWithUserId(userId.value.toString(),"Tatlı",1, 7)
                if(response.isSuccessful){
    ...
    fun controlDrinksFoodsLists(){
        CoroutineScope(Dispatchers.Main).launch {
            try {
                val response = krepo.getScoredCategorizeFoodsByPageWithUserId(userId.value.toString(),"İçicek",1, 7)
                if(response.isSuccessful){
    ...
    var mainFoodsList = Pager(PagingConfig(1)){
        CategorizedScoredFoodPagination(userId.value.toString(),krepo,"Ana Yemek")
    }.flow.cachedIn(viewModelScope)

    var soapFoodsList = Pager(PagingConfig(1)){
        CategorizedScoredFoodPagination(userId.value.toString(),krepo,"Çorba")
    }.flow.cachedIn(viewModelScope)

    var desertFoodList = Pager(PagingConfig(1)){
        CategorizedScoredFoodPagination(userId.value.toString(),krepo,"Tatlı")
    }.flow.cachedIn(viewModelScope)

    var drinksFoodList = Pager(PagingConfig(1)){
        CategorizedScoredFoodPagination(userId.value.toString(),krepo,"İçicek")
    }.flow.cachedIn(viewModelScope)
